...
struct WindowData
{
    ...
    std::queue<int> kbd_queue;// Queue of keyboard characters
    ...
    int mousedbl;
    int nmouse_key;
    int flags;			// keyboard flags
    std::queue<POINTS> clicks[WM_MOUSELAST - WM_MOUSEFIRST + 3];   // Array to hold the coordinates of the clicks
    bool mouse_queuing[WM_MOUSELAST - WM_MOUSEFIRST + 3]; // Array to tell whether mouse events should be queued
    Handler mouse_handlers[WM_MOUSELAST - WM_MOUSEFIRST + 3];   // Array of mouse event handlers
...
extern COLORREF BGI__Colors[17];  // The RGB values for the Borland 16 colors, misc.cpp
